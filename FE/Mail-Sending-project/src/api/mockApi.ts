...
function ensureSeed() {
  ...
  writeJson<Record<string, string>>(LS_SESSIONS, {})
}
...
export const mockApi = {
  async register(payload: { name: string; email: string; password: string }): Promise<AuthResponse> {
    ...
    if (password.length < 6) throw new MockApiError('Password must be at least 6 characters')
  ...
  async login(payload: { email: string; password: string }): Promise<AuthResponse> {
    ...
    if (!user || user.password !== password) throw new MockApiError('Invalid email or password', 401)
  ...
  async changePassword(payload: {
    ...
    if (newPassword.length < 6) throw new MockApiError('New password must be at least 6 characters')
    ...
    if (users[idx].password !== currentPassword) throw new MockApiError('Current password is incorrect', 400)

    users[idx] = { ...users[idx], password: newPassword }
    writeJson(LS_USERS, users)
  },

  async updateProfile(payload: { token: string; name: string }): Promise<MockUser> {
    ensureSeed()
    await sleep(350)
